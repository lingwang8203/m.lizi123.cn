	...
	$.ajax({
		...
		success:function(data){
			...
			if (obj.status==200) {
				...
				html+="<li class='aui-list-header'>选择投票选手</li>";
				for(var i=0;i<obj.user_num;i++){
					...
					html+="<a target='_blank' href='http://m.lizi123.cn/5_mine/51_mine_homepage/51_mine_homepage.html?user_id="+obj[i].user_info.id+"'><img src='"+obj[i].user_info.head_image+"' class='aui-img-round aui-list-img-sm'>";
		...
		if (user_id.length>0) {
			...
			$.ajax({
				...
				success:function(data){
					...
					console.dir(obj);
		  			if (obj.status==200) {
		  				window.location.href="31230_publish_sports_step2_Manage_frame3_publishLatest.php?club_id="+club_id;
					}else if(obj.status==199){
						window.open("http://m.lizi123.cn/7_login/7_login.html");
					}
				},
