...
<html>
		...
		real_content = real_content.replace(/\@]/g, ".gif /><span>"); 
	 	if ( file !="null" && file != "" && file!=null) {
			if(flag == 1){
				...
				$("#chat").append('<div class="mytalk"><a href="http://m.lizi123.cn/5_mine/50_mine_homepage_self/50_mine_homepage_self.html?user_id='+from_client_id+'"><img src="'+user_image+'" /></a><div class="lft"><div class="lftr"><span>'+time+'</span><h3> '+from_client_name+' </h3></div><img src="'+file+'"/></div></div>');
			}
			else{
				...
				$("#chat").append('<div class="yourtalk"><a href="http://m.lizi123.cn/5_mine/51_mine_homepage/51_mine_homepage.html?user_id='+from_client_id+'"><img src="'+user_image+'" /></a><div class="rgt"><div class="rgtl"><h3> '+from_client_name+' </h3><span>'+time+'</span></div><img src="'+file+'"/></div> </div>');
			}
			}else{
		 //无本地图片时的文字内容
		 if(flag == 1){
			 ...
			 $("#chat").append('<div class="mytalk"><a href="http://m.lizi123.cn/5_mine/50_mine_homepage_self/50_mine_homepage_self.html?user_id='+from_client_id+'"><img src="'+user_image+'" /></a><div class="lft"><div class="lftr"><span>'+time+'</span><h3> '+from_client_name+' </h3></div><div class="biaoqing_chat"><span>'+real_content+'</span></div></div></div>');
			 }else{
	 			$("#chat").append('<div class="yourtalk"><a href="http://m.lizi123.cn/5_mine/51_mine_homepage/51_mine_homepage.html?user_id='+from_client_id+'"><img src="'+user_image+'" /></a><div class="rgt"><div class="rgtl"><h3> '+from_client_name+' </h3><span>'+time+'</span></div><div class="your_biaoqing_chat"> '+real_content+'</div></div> </div>');
			 }}
  ...
  <script src="http://res.wx.qq.com/open/js/jweixin-1.0.0.js"></script>
	...
	<body onLoad="connect();">
		<header>
			<a class="title" href="new_chat_homepage.php"><</a>
...
WHERE to_user_id='$from_user_id' and from_user_id='$to_user_id' ORDER BY time ASC");
					...
					//获取两个人的nickname与头像
					$result_name = mysql_query("SELECT * FROM lz_user
WHERE id='$from_user_id'");
					while($row_name = mysql_fetch_array($result_name)){
						$from_nickname = $row_name['user_name'];
						$from_image = $row_name['head_image'];
						}
					$result_name = mysql_query("SELECT * FROM lz_user
WHERE id='$to_user_id'");
					while($row_name = mysql_fetch_array($result_name)){
						$to_nickname = $row_name['user_name'];
						$to_image = $row_name['head_image'];
						}
					//将自己的头像的赋值
					?>
                    <script>
						...
						<?php
							if($result[$i]['from_user_id'] == $from_user_id){
								...
								<?php
									...
									mysql_query("UPDATE lz_chat_dialog SET have_read = '1' WHERE id = '$mysql_id'");
									}
						?>
