...
var first_img = getQueryString('first_img');

first_img = parseInt(first_img) + 1;

var image_length = getQueryString('image_length');

//alert('first_img=' + first_img + ' the length=' + image_length);

var images = new Array(image_length);

for(var i = 0; i < image_length; i++) {
	
	images[i] = getQueryString(i);
	//alert(images[i]);
}
...
for(var i = 0; i < image_length; i++) {
	html += '<li><a href="javascript:history.go(-1);">';
	html += '<img src="'+images[i]+'" /></a>';
	html += '</li>';
}
var pic_view = document.getElementById("pic-view");
pic_view.innerHTML = html;
...
photoSlider.prototype.createSprite = function(){
	...
	imgObj.src=this.lists[this.listLength-1].getElementsByTagName('img')[3].src;
	var _this=this;
...
photoSlider.prototype.init = function() {
	...
	this.index = 0; //index是列表点的下标
	//初始化lists值
	for (var i = 0; i < this.listLength; i++) {  //i表示初始化图片的下标
...
//		}
        if(listImg.width>this.wrapWidth){
        	listImg.style.width=this.wrapWidth + 'px';
