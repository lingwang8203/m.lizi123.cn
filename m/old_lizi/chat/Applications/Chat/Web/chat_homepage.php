...
<html>
	...
	<body>
		<header>
			<a class="title" href="http://m.lizi123.cn/ground/index.html"><</a>
			<span>聊天</span>
...
WHERE to_user_id='$user_id' ORDER BY time DESC");
	...
	for($i=0;$i<$friend_num;$i++){
		...
		$result_friend = mysql_query("SELECT * FROM qj_user
WHERE user_id='$friend_id'");
		while($row = mysql_fetch_array($result_friend)){
			$friend[$i]['nickname'] = $row['user_nickname'];
			$friend[$i]['image'] = $row['user_image'];
			}
		}
	//获得我的所有群
	$result_my_group = mysql_query("SELECT * FROM qj_group_people
...
WHERE group_id = '$group_id' ORDER BY time DESC");
			while($row = mysql_fetch_array($result_group_chat)){
				...
				$result_group_chat_nickname = mysql_query("SELECT * FROM qj_user WHERE user_id = '$group_chat_user_id'");
		...
		mysql_close($con);
	//将群和私聊合并为一个数组,type:0为私聊,1为群聊
	$sum = 0;
	for($i=0;$i<$friend_num+$group_num;$i++){
		$sum++;
		if($i<$friend_num){
			$result_all[$i]['type'] = 0;
	...
	//显示聊天对象
	for($i=0;$i<$sum;$i++){
		if($result_all[$i]['type'] == 0){
			?>
            <a href="http://m.lizi123.cn/chat/Applications/Chat/Web/siliao.php?id=<?php echo $result_all[$i]['user_id']; ?>&id2=<?php echo $user_id; ?>" tartget="_blank">
            <div class="chat">
