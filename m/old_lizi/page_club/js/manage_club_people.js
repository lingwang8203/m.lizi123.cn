...
window.onload=function(){
	...
	for(var i=0;i<fenzu_label.length;i++){
		...
		fenzu_label[i].addEventListener("touchstart",function(e){
			e.preventDefault();
			if(this.index==0){
				this.src="img/sanjiao1.png";
				this.parentNode.nextElementSibling.style.display="block";
				this.index=1;
			}
			else{
				this.src="img/sanjiao2.png";
	...
	for(var j=0;j<choose.length;j++){
		...
		choose[j].addEventListener("touchstart",function(e){
			e.preventDefault();
			if(this.index==0){
				this.src="img/choose2.png";
				this.index=1;
			}
			else{
				this.src="img/choose1.png";
				this.index=0;
			}
		},false);
	}
...
//			}
	...
	yellow.addEventListener("touchstart",function(e){
		e.preventDefault();
		if(this.index==0){
			this.src="img/yellow1.png";
			this.index=1;
			cyellow.src="img/cyellow.png";
			blue.src="img/blue.png";
			tblue.src="img/tblue.png";
			red.src="img/red.png";
			cyellow.index=0;
			blue.index=0;
			tblue.index=0;
	...
	function fuyuan(){
		for(var i=0;i<choose_fenzu.length;i++){
			choose_fenzu[i].src="img/choose1.png";
...
//		var html="<div class='fenzu'>";
		var html="<img src='img/choose1.png' class='choose_fenzu'/>";
		html+="<span class='fenzu_nm'>"+newfenzu+"</span>";
		html+="<img src='../ffunction/img/edit_label/cut.png' class='edit_fenzu'>";
//		html+="</div>";
		div.innerHTML=html;
		fenzu_list.appendChild(div);
		var hhtm="<div class='list'><div class='list_header'><img src='img/sanjiao2.png' class='fenzu_icon'/><span class='fenzu_name'>"+newfenzu+"</span></div><div class='people_list'></div></div>";
		...
		if(piliang.index==1){
			for(var i=choose.length-1;i>=0;i--){
				if(choose[i].index==1){
					...
					var txt="<div class='people'><img src='img/touxiang.png' class='people_touxiang'/><img src='img/choose1.png' class='choose' style='display: none;'/><span class='"+classname+"'>"+touxian+"</span><span class='people_name'>"+name+"</span></div>";
			...
			for(var i=people.length-1;i>=0;i--){
				if(people[i].index==1){
					...
					var txt="<div class='people'><img src='"+p_touxiang+"' class='people_touxiang'/><img src='img/choose1.png' class='choose' style='display: none;'/><span class='"+classname+"'>"+touxian+"</span><span class='people_name'>"+name+"</span></div>";
					$(".people_list").eq(xinfenzu_num).append(txt);
					people[i].parentNode.removeChild(people[i]);
...
function obtainGroupInfo(club_id){
	if (club_id) {
		$.ajax({
		    type:"post",
		    url:"http://qj_api.qdmedia.cc/index.php/home/club/obtainGroupInfo",
		    data:{
		        "club_id":club_id,
		    },
...
function groupInfoShow(obj){
	...
	$("#fenzu_list").text("");
	for (var i = 0; i < obj.groupNum; i++) {
		var html = "<div class='list'>";
		html += "<div class='list_header'>";
		html += "<img src='img/sanjiao2.png' class='fenzu_icon'/>";
		...
		for (var j = 0; j < obj[i].group.length; j++) {
			...
			html += "<img src='"+obj[i].group[j].member_info.user_image+"' class='people_touxiang'/>";
			html += "<img src='img/choose1.png' name='"+obj[i].id+"'id='"+obj[i].group[j].user_id+"'class='choose' style='display: none;'/>";
			html += "<span class='people_touxian "+obj[i].group[j].color+"'>"+obj[i].group[j].user_title+"</span>";
			html += "<span class='people_name'>"+obj[i].group[j].member_info.user_nickname+"</span>";
		...
		fenzu_list += "<img src='img/choose1.png' id='"+obj[i].id+"' class='choose_fenzu'/>";
		fenzu_list += "<span class='fenzu_nm'>"+obj[i].group_name+"</span>";
		fenzu_list += "<img src='../ffunction/img/edit_label/cut.png' id='"+obj[i].id+"'class='edit_fenzu'>";
		fenzu_list += "</div>";
...
function editUserTitle(group,id,title){
	if (id) {
		$.ajax({
		    type:"post",
		    url:"http://qj_api.qdmedia.cc/index.php/home/club/editUserTitle",
		    data:{
		    	"group":group,
		        "id":id,
		        "title":title,
...
function editUserLimit(group,id,color){
	...
	$.ajax({
	    type:"post",
	    url:"http://qj_api.qdmedia.cc/index.php/home/club/editUserLimit",
	    data:{
	    	"group":group,
	        "id":id,
	        "limit":limit,
...
function deleteMember(group,id){
	...
	$.ajax({
	    type:"post",
	    url:"http://qj_api.qdmedia.cc/index.php/home/club/deleteMember",
	    data:{
	    	"group":group,
	        "id":id,
	        "club_id":club_id,
...
function deleteGroup(deleteGroupId){
	...
	if(deleteGroupId.length>0){
		...
		$.ajax({
		    type:"post",
		    url:"http://qj_api.qdmedia.cc/index.php/home/club/deleteGroup",
		    data:{
...
function addGroup(newfenzu){
	...
	$.ajax({
	    type:"post",
	    url:"http://qj_api.qdmedia.cc/index.php/home/club/addGroup",
	    data:{
	    	"newfenzu":newfenzu,
	    	"club_id":club_id,
	    },
...
function moveGroup(groupId,id,moveGroupId){
	...
	$.ajax({
	    type:"post",
	    url:"http://qj_api.qdmedia.cc/index.php/home/club/moveGroup",
	    data:{
	    	"groupId":groupId,
	    	"user_id":id,
