window.onload=function(){
	...
	///////////////////////发表情
		...
		for(var i=0;i<biaoqing.length;i++){
			...
			biaoqing[i].addEventListener("touchstart",function(e){
				e.preventDefault();
				if(this.index<=9){
				document.getElementById("message").value+="[@F_0"+(this.index+1)+"@]";
				}
				else{
				document.getElementById("message").value+="[@F_"+(this.index+1)+"@]";
				}
			},false);
		}
	...
	fasong.addEventListener("touchstart",function(e){
		...
		if(find()==false){
		var html="<div class='mytalk'><img src='img/touxiang.png' /><div class='lft'><div class='lftr'><span>2016-2-14 10:38:24</span><h3>小栗子</h3></div><span class='me_message'>"+message+"</span></div></div>";
		$("#chat").append(html);
		}
		else{
...
//              document.getElementById("add_logo").style.backgroundImage="none",
				...
				var html="<div class='mytalk'><img src='img/touxiang.png' /><div class='lft'><div class='lftr'><span>2016-2-14 10:38:24</span><h3>小栗子</h3></div><img src='"+preview+"' /></div></div>";
				$("#chat").append(html);
				var h = document.documentElement.scrollHeight || document.body.scrollHeight;
				window.scrollTo(h,h);
//          else {
