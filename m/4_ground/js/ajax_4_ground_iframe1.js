...
$(document).ready(function(){
	...
	$.ajax({
		type:"post",
		url:"http://api.lizi123.cn/index.php/home/ground/groundNew",
		xhrFields:{
...
//			alert(document.body.scrollHeight);
			window.parent.h1=document.body.scrollHeight;
		}
...
function resShow(obj){
	if (obj) {
		for(var i=0;i<obj.class.length;i++){
	        ...
	        html+="<i class='aui-iconfont aui-icon-like' name='class_dianzan'></i><span class='aui-font-size-12' style='padding-left:4px !important;'>"+obj.class[i].good_num+"</span></div>";
	        ...
	        html+="<span class='aui-font-size-12'style='padding-left:4px !important;'>"+obj.class[i].evaluate_num+"</span></div></a><div class='aui-col-xs-4 aui-pull-right'>";
		...
		for (var k = 0 ; k< obj.show.length; k++) {
			...
			if(obj.show[k].images.length == 1) {
				...
				html += "<img src='" + obj.show[k].images[0] + "' style='height:9rem;width: 70%;' /> ";
				html += "</div>";
...
function divLocation(html,time){
	if (html&&time) {
		var time_list = document.getElementsByName('time');
		console.dir(time_list);
		//alert(time_list.length);
		if (time_list.length==0) {
			$("#ground").append(html);
		}else if(time_list.length=1){
			var temp = Date.parse(new Date(time))/1000;
			var beforeTime = Date.parse(new Date(time_list[0].id))/1000;
		 	if (temp<beforeTime) {
		 		var jquery = "#"+time_list[0].id;
		 		$(jquery).before(html);
		 	}else{
			 	var jquery = "#"+time_list[0].id;
			 	$(jquery).after(html);
			...
			var temp = Date.parse(new Date(time))/1000;
			for (var m = 0; m < time_list.length; m++) {
				var beforeTime = Date.parse(new Date(time_list[m].id))/1000;
				var n = m+1;
				if (time_list[n].id) {
					var afterTime = Date.parse(new Date(time_list[n].id))/1000;
				 	if (temp<=afterTime&&temp>=beforeTime) {
				 		var jquery = "#"+time_list[n].id;
				 		$(jquery).before(html);
				 		break;
				 	}
				 }else{
				 	var jquery = "#"+time_list[m].id;
				 	$(jquery).after(html);
				 	break;
				 }
